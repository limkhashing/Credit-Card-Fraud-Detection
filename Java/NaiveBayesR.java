...
/**
 * Cardzone Fraud Detection project using Naive Bayes algorithm to detect fradulent transaction.
...
//        plotPieChart();
        ...
        // change some columns type to discrete / strings value
...
//        engine.end();
    ...
    private static void loadModel() {
        ...
        System.out.println("Loading back the saved Naive Bayesian model from RData...");
        engine.eval("load(\"src/main/resources/NaiveBayes.RData\")");
    ...
    private static void performCrossValidation(int kFoldNumber) {
        System.out.println("\nStart perform cross-validation... ");
    ...
    private static void testModelPredictions() {
        System.out.println("Testing the Naive Bayesian performance by predicting onto testing set...");
        long startTime = System.currentTimeMillis(); // get the start time
        engine.eval("system.time(test_set$predictions <- predict(model, test_set))");
        System.out.println("Test Finish");
        System.out.println("Testing time on a single transaction takes " + getExecutionTime(startTime) + " milliseconds \n");
    }

    /**
     * start training onto Naive Bayes model
     */
    private static void startTraining() {
//        engine.eval("library(naivebayes)"); // naive_bayes (Multinomial)
        engine.eval("library(e1071)"); // naiveBayes (Gaussian)

        long startTime = System.currentTimeMillis(); // get the start time
        System.out.println("\nStart Training...");

        engine.eval("model <- naiveBayes(AUTHTXN_FRAUD_CHECK~., data=train_set)");
